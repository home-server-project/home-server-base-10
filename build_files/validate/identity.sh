...
case "${HOME_SERVER_BASE_CHANNEL:-}" in
    stable)
        EXPECTED_PRETTY_NAME="Home Server Base 10"
        EXPECTED_VARIANT="Stable"
        EXPECTED_PROFILE="almalinux-10-minimal-plus"
        ;;
    testing)
        EXPECTED_PRETTY_NAME="Home Server Base 10 Testing"
        EXPECTED_VARIANT="Testing"
        EXPECTED_PROFILE="almalinux-10-minimal-plus"
        ;;
    next)
        EXPECTED_PRETTY_NAME="Home Server Base 10 Next"
        EXPECTED_VARIANT="Next"
        EXPECTED_PROFILE="almalinux-kitten-10-minimal-plus"
        ;;
    *)
        fail "HOME_SERVER_BASE_CHANNEL=${HOME_SERVER_BASE_CHANNEL:-unset}; expected stable, testing, or next"
        ;;
esac
...
[[ "${PRETTY_NAME:-}" == "${EXPECTED_PRETTY_NAME}" ]] || fail "PRETTY_NAME=${PRETTY_NAME:-unset}; expected ${EXPECTED_PRETTY_NAME}"
...
[[ "${VARIANT_ID:-}" == "${HOME_SERVER_BASE_CHANNEL}" ]] || fail "VARIANT_ID=${VARIANT_ID:-unset}; expected ${HOME_SERVER_BASE_CHANNEL}"
...
[[ "${HOME_SERVER_BASE_PROFILE:-}" == "${EXPECTED_PROFILE}" ]] || fail "base profile metadata is ${HOME_SERVER_BASE_PROFILE:-unset}; expected ${EXPECTED_PROFILE}"

if [[ "${EXPECTED_PROFILE}" == "almalinux-kitten-10-minimal-plus" ]]; then
    [[ "${HOME_SERVER_BASE_UPSTREAM_PRETTY_NAME:-}" == *"AlmaLinux Kitten"* ]] || fail "upstream PRETTY_NAME metadata is not AlmaLinux Kitten"
else
    [[ "${HOME_SERVER_BASE_UPSTREAM_PRETTY_NAME:-}" != *"AlmaLinux Kitten"* ]] || fail "normal AlmaLinux channel contains Kitten upstream metadata"
fi
...
if [[ -e "${OS_RELEASE_ETC}" ]] && ! [[ "${OS_RELEASE_ETC}" -ef "${OS_RELEASE_USR}" ]]; then
    for key in ID NAME PRETTY_NAME VARIANT VARIANT_ID IMAGE_ID IMAGE_VERSION VENDOR_NAME CPE_NAME HOME_SERVER_BASE_UPSTREAM_ID HOME_SERVER_BASE_PROFILE HOME_SERVER_BASE_CHANNEL; do
...
pass "Home Server Base 10 ${EXPECTED_VARIANT} identity and upstream metadata"

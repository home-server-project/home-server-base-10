...
# Capture the true upstream Kitten foundation before replacing the visible
# product identity. AlmaLinux Kitten remains provenance, not the resulting OS
# product identity.
...
[[ "${BASE_PRETTY_NAME}" == *"AlmaLinux Kitten"* ]] || {
    echo "ERROR: expected AlmaLinux Kitten upstream, got '${BASE_PRETTY_NAME}'." >&2
    exit 1
}
[[ "${BASE_VERSION_ID%%.*}" == "10" ]] || {
    echo "ERROR: expected AlmaLinux Kitten major version 10, got '${BASE_VERSION_ID}'." >&2
    exit 1
}
...
[[ "${IMAGE_CHANNEL}" == "next" ]] || {
    echo "ERROR: next branch must build IMAGE_CHANNEL=next, got '${IMAGE_CHANNEL}'." >&2
    exit 1
}
...
# Preserve exact upstream Kitten provenance under Home Server Project-owned
# fields so future compatibility failures can be tied back to the source.
...
osr_set HOME_SERVER_BASE_PROFILE "almalinux-kitten-10-minimal-plus"
